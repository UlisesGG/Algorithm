...
def restriccion(A, B, ListO, p, q):
    for i in range(p, q):
        if(((A.x<ListO[i].x and B.x>ListO[i].x) and (A.y>ListO[i].y and B.y<ListO[i].y)) or ((A.x<ListO[i].x and B.x>ListO[i].x) and (A.y<ListO[i].y and B.y>ListO[i].y))):
            return False
    return True
        ...
        if(aparear[i]==0):
            for j in range(i+1, len(lPuntos)):
                if(aparear[j]==0):
                    ...
                    if(restriccion(lPuntos[i], lPuntos[j], lPuntos, r, q) and dim>math.sqrt(((lPuntos[j].x-lPuntos[i].x)**2)+((lPuntos[j].y-lPuntos[i].y)**2)) and lPuntos[i].color==lPuntos[j].color):
